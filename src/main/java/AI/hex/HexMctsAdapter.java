...
public final class HexMctsAdapter implements MctsGame<HexState> {
    ...
    public HexMctsAdapter(int n) { this.n = n; }

    @Override public int actionSize() { return n * n; }

    @Override
    public boolean[] getValidMoves(HexState s) {
        boolean[] valid = new boolean[n * n];
        for (int a = 0; a < valid.length; a++) valid[a] = (s.cells[a] == 0);
        return valid;
    }
    ...
    public HexState getNextState(HexState s, int action, int player) {
        ...
        next[action] = (player == 1) ? +1 : -1;   // <-- key line: respect player sign
        return new HexState(s.n, next, s.plusIsRed);
    }

    @Override
    public HexState changePerspective(HexState s, int player) {
        // Your MCTS calls this with player == -1 when handing the turn to the opponent.
        if (player != -1) throw new IllegalArgumentException("Expected player==-1");
        return s.flippedPerspective();
    }

    @Override
    public Outcome valueAndTerminated(HexState s, Integer lastAction) {
        // Rebuild a physical Board to test connectivity
        Board b = new Board(n);
        for (int idx = 0; idx < s.cells.length; idx++) {
            ...
            if (v == +1) {
                if (s.plusIsRed) b.getMoveRed(r, c, Color.RED);
                else             b.getMoveBlack(r, c, Color.BLACK);
            } else { // v == -1
                if (s.plusIsRed) b.getMoveBlack(r, c, Color.BLACK);
                else             b.getMoveRed(r, c, Color.RED);
            }
        }

        boolean red = b.redWins();
        boolean black = b.blackWins();
        if (!red && !black) return new Outcome(0.0, false);
    ...
    @Override public int opponent(int player) { return -player; }

    @Override public double opponentValue(double v) { return -v; }
